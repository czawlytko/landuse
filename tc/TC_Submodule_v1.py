...
import dense_mp_v1 as env_pkg
from dense_mp_v1 import dense as callDense
import QGIS_geoprocessing as qgis_pkg
import shutil
...
def runTCT(args):
    ...
    """
    psegs, allAg, allTurf, tile, cf = args
    crs = psegs.crs
    ...
    ag = getSubsetAg(psegs, 4, allTurf, allAg, tile, cf) #returns 3rd argument - tct_forested to calc tct and forest for forest frag
    ...
    tct = []
    tct.append(calcTCT4(dense[0], roads, 20, 1, tile, cf)) #dense tct
    del dense[0] #gets rid of gdf - leaves forest
    tct.append(calcTCT4(notDense[0], roads, 10, 2, tile, cf)) #not dense tct
    del notDense[0] #get rid of gdf - leaves forest
    tct.append(calcTCT4(forested[0], roads, 10, 3, tile, cf)) #not dense tct - roads_union
    del forested[0] #gets rid of gdf - leaves forest
    tct.append(calcTCT4(ag[0], roads, 10, 4, tile, cf)) #not dense tct
    ...
    forest = groupForest(forest, tct, tile, cf)

    note = str(tile) + " -- Removed TCT Buffers and Dissolved Forest -- "
    etime(cf, note, st)
    st = time.time()

    toa =  getForestFrag(forest, psegs, allTurf, tile, cf)
...
def calcTCT4(gdf, roads, bufferSize, envType, tile, cf):
    """
    Method: calcTCT4()
    ...
    if len(gdf) > 0:
        ...
        input_layer = f"B:/landuse/rev1/{cf}/temp_qgis_files/temp_" + layer_name + ".gpkg|layername=" + layer_name
        output_layer = f"B:/landuse/rev1/{cf}/temp_qgis_files/" + layer_name+".gpkg"
        gdf.to_file(f"B:/landuse/rev1/{cf}/temp_qgis_files/temp_" + layer_name + ".gpkg", layer=layer_name, driver="GPKG")
        gdf = qgis_pkg.dissolve(input_layer, output_layer, True, fields=['PID'])
        gdf = gdf[['PID', 'geometry']]
        note = str(tile) + "--Unioned gdf"
        etime(cf, note, st)
        st = time.time()
        gdf = bufIntBuilding(gdf, buildings)
        ...
        gdf = spatial_overlay_mp(gdf, roads, 'difference', 'PID')
        ...
        if len(gdf) > 0:
            ...
            gdf.to_file(f"B:/landuse/rev1/{cf}/temp_qgis_files/temp_" + layerName + ".gpkg", layer=layerName, driver='GPKG')
            # input_layer = f"B:/landuse/rev1/{cf}/temp_qgis_files/temp_" + layerName + ".gpkg|layername=" + layerName
            # output_layer = os.path.join(f"B:/landuse/rev1/{cf}/temp_qgis_files", layerName + ".gpkg")
            # qgis_pkg.dissolve(input_layer, output_layer, False, fields=None)
            # st = time.time()
            # note = str(tile) + "--Dissolved buffers"
            # etime(cf, note, st)
            # st = time.time()
            # return f"B:/landuse/rev1/{cf}/temp_qgis_files/" + layerName + ".gpkg|layername=" + layerName 
            return f"B:/landuse/rev1/{cf}/temp_qgis_files/temp_" + layerName + ".gpkg|layername=" + layerName
    return ''
...
def getForestFrag(under_acre, psegs, allTurf, tile, cf):
    """
    ...
    if len(under_acre) > 0:
        ...
        sb_nonag = shared_border_mp(psegs[psegs['lu'].isin(dev)][['PSID', 'geometry']], under_acre, 'maj')
        ...
        sb_nonag = shared_border_mp(psegs[psegs['lu'].isin(dev)][['PSID', 'geometry']], over_acre, 'maj')
        ...
        isWB = getWidth_mp(over_acre_nodev, 'Id')
...
def getSubsetEnv(gdf, envType, allTurf, tile, cf):
    ...
    if len(forest) > 0:
        ...
        input_layer = f"B:/landuse/rev1/{cf}/temp_qgis_files/temp_" + for_layer_name + ".gpkg|layername=" + for_layer_name
        output_layer = f"B:/landuse/rev1/{cf}/temp_qgis_files/" + for_layer_name + ".gpkg"
        forest.to_file(f"B:/landuse/rev1/{cf}/temp_qgis_files/temp_" + for_layer_name + ".gpkg", layer=for_layer_name, driver="GPKG")
...
def getSubsetAg(gdf, envType, allTurf, allAg, tile, cf):
    """
    Method: getSubsetAg()
    ...
    sb_ag = shared_border_mp(gdf[gdf['lu'].isin(allAg)][['PSID', 'geometry']], forest, 'all_ag')
    ...
    if len(forest) > 0:
        ...
        tmp = sjoin_mp6(gdf[['lu', 'geometry']], 15000, 'intersects', ['Id', 'lu'], forest[['Id', 'geometry']])
        ...
        forest.to_file(f"B:/landuse/rev1/{cf}/temp_qgis_files/" + for_layer_name + ".gpkg", layer=for_layer_name, driver="GPKG")
...
def bufIntBuilding(gdf, buildings):
    """
    ...
    """
    idxToRemove = polyContains_mp(gdf, buildings, 'PID')
...
def deleteTempFiles(cf):
    ...
    Returns: N/A
    """
    qgis_temp_files_path = f"B:/landuse/rev1/{cf}/temp_qgis_files"
...
def etime(cf, note, starttime):
    ...
    f = open(f"B:/landuse/rev1/{cf}/etime_treesover_log.txt", "a")
    if elapsed > 60:
...
def groupForest(forest_list, tct_list, tile, cf):
    """
    Method: groupForest()
        ...
        if os.path.isfile(forest_list[f]): #if there is forest of this env type in tile
            ...
            if os.path.isfile(tct_list[f].split('|layername=')[0]): #if there are tct bufs of this env type in tile
                ...
                if f+1 == 4: # if ag env
                    ...
                    forest = spatial_overlay_mp(tmp_out, unary_union(overlayGDF['geometry']), 'difference', 'Id')
                    ...
                    forest = spatial_overlay_mp(forest, unary_union(overlayGDF['geometry']), 'difference', 'Id')
...
def polyContains_mp(gdf1, gdf2, idField):
    """
...
def shared_border_mp(ag_segs, forest, r_field):
    """
    ...
    """
    global NUM_CPUS
    ag_for = sjoin_mp6(ag_segs, 10000, 'intersects', ['Id', 'PSID'], forest)
    if len(ag_for) > 0:
...
#####################################################################################
def spatial_overlay_mp(gdf, overlayGDF, operation, Id):
    """
...
# GLOBAL number of tiles to run simulataneously
NUM_CPUS = 5 #optimal number of processers for each subprocess?

#Calculate number of tiles to run at once
NUM_TILES = int((mp.cpu_count() - 2) / (NUM_CPUS + 1))

badGeoms = f"B:/landuse/rev1/wico_24045/temp_qgis_files/bad_geoms.gpkg"
...
if __name__ == "__main__":
    ...
    psegs_layer = 'psegs_lu'
    parcels_layer = 'parcels_vectorized'
    urban_shp = r"B:/ancillary/census/urban_area_albers.shp"

    for cf in cflist:
        psegs_gpkg = f"B:/landuse/rev1/{cf}/output/data.gpkg" # #  full county with "corrected" geoms for testing 
        parcels_gpkg = f"B:/landuse/rev1/{cf}/temp/temp_dataprep.gdb" 
        out_gpkg = f"B:/landuse/rev1/{cf}/output/trees_over.gpkg" 
        tiles_shp = f"B:/landuse/rev1/{cf}/temp/tc_tile_buff.shp" #tile for cumb - tiles for berk
        ...
        # try: #if a county fails - don't kill following counties
        start = time.time()
        ...
        for idx, row in tiles.iterrows():
            psegs = env_pkg.readPolysMask(psegs_gpkg, psegs_layer, row['geometry'].envelope) #read in psegs layer
            ...
            chunk_iterator.append( (psegs, allAg, allTurf, row['Id'], cf) )
