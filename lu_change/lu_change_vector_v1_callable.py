...
def runNewStructure(lc_change_gdf, parcels_gdf, lc_change_ras_path, lc_change_dict, tab_area_df, lu_2017_ras_path, tct_gdf, raster_parcels_path):
    ...
    lc_change_gdf = lc_change_gdf.merge(parcel_groups[['PID', 'GID']], how='left')

    # Get majority TYPE for each unique GID
    gids = list(set(list(parcel_groups['GID'])))
    for gid in gids:
        # if some are equal counts, use mode to get list to give precendence to ag classes
        types = list(lc_change_gdf[lc_change_gdf['GID'] == gid][['TYPE']].mode()['TYPE']) 
        if 'crop' in types:
            parcel_groups.loc[parcel_groups['GID'] == gid, 'GRP_TYPE'] = 'crop'
        elif 'pasture' in types:
            parcel_groups.loc[parcel_groups['GID'] == gid, 'GRP_TYPE'] = 'pasture'
        else:
            parcel_groups.loc[parcel_groups['GID'] == gid, 'GRP_TYPE'] = 'other'
    
    lc_change_gdf = lc_change_gdf.merge(parcel_groups[['PID', 'GRP_TYPE']], how='left')
...
def runIndirect(lc_change_gdf, psegs, lu_2017_ras_path, t1_tc_gdf):
    ...
    lu_list_vals = list(lch.get_lu_code('ALL', False).values())
...
def run_lu_change(cf, lu_type):
    ...
    try: # if a county fails don't disrupt other counties
        ...
        project_folder = f"{folder}\{cf}\LandUseChange"
        main_path = os.path.abspath(f"{folder}/{cf}")
        lu_chg_path = os.path.join(main_path, 'LandUseChange')
        input_path = os.path.join(lu_chg_path, 'input')
        ...
        for idx, row in change_df.iterrows():
            lc_change_dict[row['Description']] = row['Value']
        ...
        lc_change_meta.update({'nodata':0,
                                'dtype':'uint16'})
        with rio.open(os.path.join(output_path, f"{cf}_T1_LU_{lu_type}.tif"), 'w', **lc_change_meta, compress="LZW") as dataset:
        ...
        # Create P6 LU Change
        p6_change.run_p6_rollup_change(cf, lu_type, anci_folder, output_path) 
